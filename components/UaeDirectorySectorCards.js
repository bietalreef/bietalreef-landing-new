...
export default function UaeDirectorySectorCards({ emirate, locale = 'ar' }) {
  ...
  const hrefFor = (slug) => isEn ? `/en/uae/${emirate.slug}/sectors/${slug}` : `/uae/${emirate.slug}/${slug}`;
  ...
  return (
    <section id="uae-sector-cards" className="mx-auto max-w-6xl px-4 py-14 md:py-18">
      <div className={isEn ? 'mb-8 text-center md:text-left' : 'mb-8 text-center md:text-right'}>
        <span className="inline-flex rounded-full border border-[#B8922B]/30 bg-white px-4 py-1.5 text-xs font-black text-[#8A6A00] shadow-sm">{isEn ? '7 main sectors' : '7 قطاعات رئيسية'}</span>
        <h2 className="mt-4 text-3xl font-black text-[#0F3F1A] md:text-4xl">{isEn ? `Choose the sector in ${emirateName}` : `اختر القطاع في ${emirateName}`}</h2>
        <p className="mx-auto mt-3 max-w-3xl text-sm font-semibold leading-8 text-gray-600 md:mx-0 md:text-base">{isEn ? 'Only the seven approved sectors appear here. Extra specialties remain organized inside the UAE Directory smart footer.' : 'تظهر هنا فقط القطاعات السبعة المعتمدة. أما التخصصات الإضافية فتبقى منظمة داخل الفوتر الذكي لدليل الإمارات.'}</p>
      </div>
        ...
        {directorySectors.map((card, index) => {
          ...
          return (
            <article key={`${card.slug}-${index}`} className="group overflow-hidden rounded-[2rem] border border-[#E6DCC8] bg-white shadow-[0_18px_45px_rgba(18,58,70,0.08)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_60px_rgba(18,58,70,0.15)]">
              <div className="relative h-48 overflow-hidden bg-[#F5EFE4] sm:h-52"><Image src={card.image} alt={title} fill className="object-cover transition duration-700 group-hover:scale-105" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" /><div className="absolute inset-0 bg-gradient-to-t from-[#0F3F1A]/72 via-[#0F3F1A]/18 to-transparent" /><div className="absolute bottom-4 right-4 left-4 flex items-center justify-between gap-3"><span className="rounded-full border border-[#D4AF37]/45 bg-white/84 px-3 py-1.5 text-[11px] font-black text-[#0F3F1A] shadow-lg backdrop-blur-xl">{eyebrow}</span><Sparkles className="h-5 w-5 text-[#F7E7A0] drop-shadow" aria-hidden="true" /></div></div>
              <div className="p-5 md:p-6"><h3 className="text-xl font-black leading-8 text-[#0F3F1A]">{title}</h3><p className="mt-3 min-h-[76px] text-sm font-semibold leading-7 text-gray-600">{desc}</p><div className="mt-4 flex flex-wrap gap-2">{tags.map((tag) => <span key={tag} className="rounded-full bg-[#FDF7E8] px-3 py-1 text-[11px] font-black text-[#8A6A00]">{tag}</span>)}</div><Link href={hrefFor(card.slug)} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-[#123A46] px-5 py-3 text-sm font-black text-white shadow-[0_10px_0_rgba(18,58,70,0.12)] transition hover:bg-[#D4AF37] hover:text-[#0F3F1A]">{isEn ? 'Open sector' : 'افتح القطاع'}{isEn ? <ChevronRight className="h-4 w-4" aria-hidden="true" /> : <ChevronLeft className="h-4 w-4" aria-hidden="true" />}</Link></div>
            </article>
          );
        })}
      </div>
