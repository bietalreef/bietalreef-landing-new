...
export default function AlRehabEnglishPage() {
  ...
  const profileWhatsapp = buildProviderWhatsappUrl({
    phone: whatsapp,
    locale: 'en',
    providerName: provider.identity.name.en,
    providerCode: provider.id,
    location: 'Al Ain · Abu Dhabi · Dubai',
    summary: provider.description.en,
    profilePath: `/en/providers/${provider.slug}`,
  });
  const schemas = [
    ...
    <>
      ...
      <div dir="rtl" className="min-h-screen bg-[#F7FAFC] text-[#082E63]">
        ...
        <main dir="ltr" className="min-h-screen bg-[#F7FAFC] text-left text-[#082E63]">
          ...
          <section className="mx-auto max-w-6xl px-4 py-16"><p className="font-black text-[#52A91B]">Professional cleaning paths</p><h2 className="mt-2 text-3xl font-black">Four clearly defined services</h2><div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{provider.services.map((service) => { const message = buildCardWhatsappUrl({ phone: whatsapp, locale: 'en', cardType: 'service', providerName: provider.identity.name.en, providerCode: provider.id, cardCode: service.id, cardId: service.id, title: service.title.en, description: service.summary.en, pricingModel: 'Price confirmed after assessment', location: 'Al Ain · Abu Dhabi · Dubai', pagePath: `/en/providers/${provider.slug}#${service.id}` }); return <article id={service.id} key={service.id} className="overflow-hidden rounded-3xl border border-blue-100 bg-white shadow-sm"><div className="relative h-48"><Image src={service.image} alt={service.title.en} fill className="object-cover" /></div><div className="p-5"><span className="text-xs font-black text-[#0B5EAE]">{service.id}</span><h3 className="mt-2 text-xl font-black">{service.title.en}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{service.summary.en}</p><a href={message} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#55B51B] px-4 text-sm font-black text-white"><MessageCircle className="h-4 w-4" />Request via WhatsApp</a></div></article>; })}</div></section>
          ...
          <section className="bg-[#EAF5FF]"><div className="mx-auto max-w-6xl px-4 py-16"><p className="font-black text-[#52A91B]">Service packages</p><h2 className="mt-2 text-3xl font-black">Four quotation-based offers</h2><p className="mt-3 text-slate-600">Final pricing is confirmed after reviewing photos, quantity, dimensions and location.</p><div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{provider.offers.map((offer) => { const message = buildCardWhatsappUrl({ phone: whatsapp, locale: 'en', cardType: 'offer', providerName: provider.identity.name.en, providerCode: provider.id, cardCode: offer.id, cardId: offer.id, title: offer.title.en, description: offer.summary.en, price: 'After assessment', pricingModel: 'Quotation after reviewing photos, quantity and location', location: 'Al Ain · Abu Dhabi · Dubai', pagePath: `/en/providers/${provider.slug}#${offer.id}` }); return <article id={offer.id} key={offer.id} className="overflow-hidden rounded-3xl bg-white shadow-sm"><div className="relative h-40"><Image src={offer.image} alt={offer.title.en} fill className="object-cover" /></div><div className="p-5"><span className="inline-flex rounded-full bg-green-50 px-3 py-1 text-xs font-black text-green-700">{offer.badge.en}</span><p className="mt-3 text-xs font-black text-[#0B5EAE]">{offer.id}</p><h3 className="mt-2 text-lg font-black">{offer.title.en}</h3><p className="mt-2 text-sm leading-7 text-slate-600">{offer.summary.en}</p><p className="mt-3 font-black text-[#52A91B]">Price after assessment</p><a href={message} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#55B51B] px-4 text-sm font-black text-white"><MessageCircle className="h-4 w-4" />Inquire via WhatsApp</a></div></article>; })}</div></div></section>
