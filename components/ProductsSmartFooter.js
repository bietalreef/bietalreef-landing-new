...
const productActions = [
  { ar: 'تصفح المنتجات', en: 'Browse products', hrefAr: '#marketplace-categories', hrefEn: '#marketplace-categories', icon: '🔎' },
  { ar: 'اطلب تسعير مواد', en: 'Request material pricing', hrefAr: '/request-quote', hrefEn: '/request-quote', icon: '📋' },
  { ar: 'تصفح الموردين والمتاجر', en: 'Browse suppliers and stores', hrefAr: '/providers', hrefEn: '/en/providers', icon: '🏬' },
  { ar: 'مساعدة في التوريد', en: 'Sourcing help', hrefAr: 'https://wa.me/971567856001', hrefEn: 'https://wa.me/971567856001', icon: '💬' },
];
...
const productJourney = [
  ...
  { ar: 'أضف الكمية والمواصفات', en: 'Add quantity and specifications', hrefAr: '/request-quote', hrefEn: '/request-quote' },
  { ar: 'حدد موقع التوريد', en: 'Set the supply location', hrefAr: '/request-quote', hrefEn: '/request-quote' },
...
function LinkList({ items, locale }) {
  return (
    <div className="space-y-2">
      {items.map((item) => (
        <Link key={`${labelFor(locale, item)}-${hrefFor(locale, item)}`} href={hrefFor(locale, item)} className="flex items-center justify-between gap-3 rounded-2xl border border-[#F1E7D5] bg-[#FDFBF7] px-4 py-3 text-sm font-bold leading-6 text-gray-700 transition hover:border-[#D4AF37] hover:bg-white hover:text-[#0F3F1A]">
    ...
    <section dir={dir} className="bg-[#FDFBF7] px-4 py-12 md:py-16">
      <div className="mx-auto max-w-7xl">
        <div className="mb-7 text-center">
          <p className="text-sm font-black text-[#B8922B]">{isEn ? 'Products gateway' : 'بوابة المنتجات والمتاجر'}</p>
          <h2 className="mt-2 text-2xl font-black leading-tight text-[#0F3F1A] md:text-4xl">{isEn ? 'Smart footer for Products & Stores' : 'فوتر ذكي خاص بالمنتجات والمتاجر'}</h2>
          <p className="mx-auto mt-3 max-w-3xl text-sm font-semibold leading-7 text-gray-600 md:text-base">
            {isEn ? 'A dedicated products footer that connects materials, stores, suppliers, pricing requests and related marketplace paths.' : 'فوتر مستقل للمنتجات والمتاجر يربط المواد، المتاجر، الموردين، طلبات التسعير، ومسارات الشراء والتوريد المرتبطة.'}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {productActions.map((item) => (
            <Link key={item.hrefAr} href={hrefFor(locale, item)} className="group rounded-[1.6rem] border border-[#E6DCC8] bg-white p-4 shadow-[0_16px_36px_rgba(18,58,70,0.06)] transition hover:-translate-y-1 hover:border-[#D4AF37] hover:shadow-2xl hover:shadow-[#123A46]/10">
              ...
              <p className="mt-2 text-xs font-semibold leading-6 text-gray-500">{isEn ? 'Move to the right product and supply path.' : 'انتقل إلى المسار المناسب للمنتج أو التوريد.'}</p>
            </Link>
          ))}
        </div>

        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3">
          <FooterCard defaultOpen title={isEn ? 'Product categories' : 'فئات المنتجات'} subtitle={isEn ? 'Materials and products without page crowding' : 'مواد ومنتجات بدون ازدحام بصري'} icon="📦">
            <LinkList locale={locale} items={productGroups} />
          </FooterCard>

          <FooterCard defaultOpen title={isEn ? 'Sourcing journey' : 'مسار طلب التوريد'} subtitle={isEn ? 'From product need to supplier path' : 'من احتياج المادة إلى مسار المورد'} icon="🧭">
            <LinkList locale={locale} items={productJourney} />
          </FooterCard>

          <FooterCard title={isEn ? 'Related platform sections' : 'أقسام مرتبطة داخل المنصة'} subtitle={isEn ? 'Services, providers and UAE directory' : 'الخدمات، المزودون، ودليل الإمارات'} icon="🏗️">
            <LinkList locale={locale} items={relatedLinks} />
          </FooterCard>
        </div>
        ...
        <div className="mt-8 rounded-[2rem] border border-[#E6DCC8] bg-white p-5 shadow-[0_18px_45px_rgba(18,58,70,0.07)] md:flex md:items-center md:justify-between md:gap-6">
          <div>
            <h3 className="text-xl font-black text-[#0F3F1A]">{isEn ? 'Need materials or products for your project?' : 'تحتاج مواد أو منتجات لمشروعك؟'}</h3>
            <p className="mt-2 text-sm font-semibold leading-7 text-gray-600">{isEn ? 'Send the product type, quantity, specifications and supply location so Biet Al Reef can guide the request correctly.' : 'أرسل نوع المنتج والكمية والمواصفات وموقع التوريد حتى يتم توجيه الطلب بشكل صحيح.'}</p>
          </div>
          <Link href="/request-quote" className="mt-5 inline-flex w-full items-center justify-center rounded-2xl bg-[#0F3F1A] px-7 py-3 text-sm font-black text-white shadow-lg transition hover:bg-[#D4AF37] hover:text-[#1F170D] md:mt-0 md:w-auto">
            {isEn ? 'Request product pricing' : 'اطلب تسعير المنتج'}
          </Link>
        </div>
      </div>
    </section>
