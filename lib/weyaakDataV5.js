...
function locationTerms(value) {
  ...
  if (normalized.includes('العين')) terms.add('ابوظبي');
  return [...terms];
}
...
async function loadProviders() {
  const [profiles, services, areas] = await Promise.all([
    supabaseGet('provider_public_profiles?select=id,provider_profile_id,slug,name_ar,name_en,provider_type,short_description_ar,short_description_en,description_ar,description_en,canonical_url,verification_status,search_keywords_ar,search_keywords_en&publication_status=eq.published&limit=100'),
    ...
    supabaseGet('provider_service_area_profiles?select=provider_profile_id,emirate,main_city,all_emirate,areas,coverage_notes&limit=500'),
  ]);

  return profiles.map((provider) => ({
    ...provider,
    services: services.filter((service) => service.provider_id === provider.id),
    areas: areas.filter((area) => provider.provider_profile_id && area.provider_profile_id === provider.provider_profile_id),
  }));
}
...
export async function searchProviders({ service, city = '', emirate = '' }) {
  ...
  const wantedLocations = locationTerms([requestedCity, requestedEmirate].filter(Boolean).join(' '));
  ...
  const matches = providers.map((provider) => {
    const serviceText = normalizeArabic([
      ...
      ...(provider.services || []).flatMap((item) => [item.title_ar, item.title_en, item.description_ar, item.description_en]),
    ].filter(Boolean).join(' '));
    ...
    const locationText = normalizeArabic([
      ...
      provider.description_ar,
      provider.description_en,
      ...(provider.areas || []).flatMap((area) => [
        area.emirate,
        area.main_city,
        ...(Array.isArray(area.areas) ? area.areas : []),
        area.coverage_notes,
        area.all_emirate ? 'كل الاماره' : '',
      ]),
    ].filter(Boolean).join(' '));

    const matchedTerms = wantedServiceTerms.filter((term) => serviceText.includes(term));
    const specialtyMatched = matchedTerms.length > 0;
    const locationMatched = wantedLocations.length === 0 || wantedLocations.some((term) => locationText.includes(term));

    // الموقع لا يمكن أن يعوّض عن تخصص خاطئ.
    if (!specialtyMatched || !locationMatched) return null;
    ...
    return {
      ...
      provider: {
        ...
        services: (provider.services || []).map((item) => item.title_ar || item.title_en).filter(Boolean).slice(0, 8),
      },
    };
...
export async function submitAction(action, meta = {}) {
  ...
  if (action.type === 'quote_request') {
    ...
    return { requestNumber: result.request_number || result.request_no || result.quote_number || fallback, payload };
  }
  ...
  return { requestNumber: result.request_number || result.request_no || result.inquiry_number || fallback, payload };
}
