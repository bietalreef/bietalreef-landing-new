        ...
        <main dir="ltr" className="-mt-[1px] flex-1 bg-[#FDFBF7] text-left">
          <section className="relative isolate overflow-hidden bg-[#FDFBF7]">
            <div className="relative min-h-[600px] overflow-hidden md:min-h-[680px] lg:min-h-[740px]">
              <Image
                ...
                alt="Construction, maintenance and finishing services and offers inside Biet Al Reef"
                fill
                priority
              ...
              <Link href="/en" className="absolute left-4 top-4 z-20 inline-flex items-center gap-2 rounded-2xl border border-[#D4AF37]/45 bg-white/76 px-3 py-2 text-xs font-black text-[#123A46] shadow-xl shadow-[#123A46]/16 backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white md:left-8 md:top-8 md:px-4 md:py-3 md:text-sm">
                <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#123A46] text-[#F7E7A0] shadow-inner"><ArrowLeft className="h-4 w-4" aria-hidden="true" /></span>
                Back to home
              </Link>
              ...
              <div className="relative z-10 mx-auto flex min-h-[600px] max-w-6xl flex-col justify-end px-4 pb-8 pt-20 md:min-h-[680px] md:pb-12 lg:min-h-[740px]">
                ...
                <h1 className="max-w-4xl text-4xl font-black leading-tight text-[#0F3F1A] drop-shadow-[0_2px_12px_rgba(255,255,255,0.85)] md:text-6xl">
                  Choose the service you need<br />and turn your request into a clear path
                </h1>
                ...
                <div className="mt-5 max-w-4xl rounded-[2.1rem] border border-white/70 bg-white/64 p-4 shadow-2xl shadow-[#123A46]/14 backdrop-blur-2xl md:p-6">
                  <p className="max-w-3xl text-sm font-bold leading-8 text-gray-700 md:text-lg md:leading-9">
                    This section starts from the type of work: contracting, maintenance, finishing, carpentry, cleaning or equipment rental. Choose what you need, then send your project details for guidance or a suitable quotation path.
                  </p>
          ...
          <section id="services-list" className="max-w-6xl mx-auto px-4 py-14 md:py-20">
            <div className="mb-8 text-center md:text-left">
              <span className="inline-flex rounded-full border border-[#B8922B]/30 bg-white px-4 py-1.5 text-xs font-black text-[#8A6A00] shadow-sm">Service sectors</span>
              <h2 className="mt-4 text-3xl font-black text-[#0F3F1A] md:text-4xl">Choose the service type</h2>
              <p className="mx-auto mt-3 max-w-3xl text-sm font-semibold leading-8 text-gray-600 md:mx-0 md:text-base">Mobile-first cards: clear image, focused content and a direct path to open each service.</p>
            </div>
                  ...
                  <Link key={service.id} href={categorySlug === 'workshops' ? '/services/workshops' : `/en/categories/${categorySlug}`}>
                    <article className="group h-full overflow-hidden rounded-[2rem] border border-[#E6DCC8] bg-white shadow-[0_18px_45px_rgba(18,58,70,0.08)] transition duration-300 hover:-translate-y-1 hover:border-[#D4AF37] hover:shadow-[0_24px_60px_rgba(18,58,70,0.15)]">
                      <div className="relative h-52 overflow-hidden bg-[#F5EFE4] sm:h-56 lg:h-52">
                        <Image src={getSectorCardImage(categorySlug)} alt={copy[0]} fill className="object-cover transition duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 33vw" />
                        <div className="absolute inset-0 bg-gradient-to-t from-[#0F3F1A]/78 via-[#0F3F1A]/16 to-transparent" />
                        <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between gap-3">
                          <span className="rounded-2xl bg-white/92 px-4 py-2 text-sm font-black text-[#0F3F1A] shadow-lg backdrop-blur-xl">{copy[0]}</span>
                          <span className="rounded-2xl bg-[#123A46] px-3 py-2 text-xs font-black text-[#F7E7A0] shadow-lg ring-1 ring-[#D4AF37]/40">Service</span>
                        </div>
                      </div>
                      <div className="p-5 md:p-6">
                        <h3 className="mb-3 text-xl font-black leading-8 text-[#0F3F1A] group-hover:text-[#B8922B]">{copy[0]}</h3>
                        <p className="mb-5 text-sm font-semibold leading-7 text-gray-600">{copy[1]}</p>
                        <div className="mb-5 rounded-2xl border border-[#E6DCC8] bg-[#FDFBF7] px-4 py-3">
                          <p className="text-xs font-black text-gray-500">Pricing method</p>
                          <p className="mt-1 text-sm font-black text-[#0F3F1A]">Based on project details</p>
                        </div>
