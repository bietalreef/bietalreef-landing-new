...
export default async function weyaakAgentHandler(req, res) {
  ...
  try {
    ...
    const instructions = buildWeyaakV5Prompt({ state: { audience: previous.audience || 'unknown', intent: previous.intent || 'general', payload: normalizePayload(previous.payload) }, governmentSources: official, pageContext });
