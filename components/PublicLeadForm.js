...
export default function PublicLeadForm({ formType = 'quote' }) {
  ...
  const description = isQuote ? 'اكتب تفاصيل مشروعك أو الخدمة المطلوبة وسنحوّل الطلب لمسار منظم داخل بيت الريف.' : 'اكتب سؤالك أو احتياجك وسنرد عليك من خلال فريق بيت الريف.';
        ...
        {status === 'success' ? (
          <div className="rounded-[1.75rem] border border-green-200 bg-green-50 p-6 text-center">
            ...
            {requestNumber ? (
              <div className="mt-4 rounded-3xl border border-green-200 bg-white p-4 shadow-sm">
                <p className="text-sm font-black leading-7 text-green-800">من فضلك احتفظ بهذا الرقم الآن، ويمكنك المتابعة به لاحقًا.</p>
                <div className="mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-center">
                  <p dir="ltr" className="rounded-2xl bg-[#FDFBF7] px-5 py-3 text-xl font-black tracking-wide text-[#0F3F1A] ring-1 ring-[#E6DCC8]">{requestNumber}</p>
                  <button type="button" onClick={copyRequestNumber} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-[#0F3F1A] px-5 py-3 text-sm font-black text-white transition hover:bg-[#D4AF37] hover:text-[#0F3F1A]">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="الاسم" required><input value={form.full_name} onChange={(e) => patch('full_name', e.target.value)} className="input" placeholder="اكتب اسمك" /></Field>
              <Field label="رقم الهاتف / واتساب" required><input value={form.phone} onChange={(e) => patch('phone', e.target.value)} className="input" dir="ltr" inputMode="tel" placeholder="05xxxxxxxx" /></Field>
              <Field label="البريد الإلكتروني"><input value={form.email} onChange={(e) => patch('email', e.target.value)} className="input" dir="ltr" type="email" placeholder="name@email.com" /></Field>
              <Field label="طريقة التواصل المفضلة"><select value={form.preferred_contact} onChange={(e) => patch('preferred_contact', e.target.value)} className="input"><option value="whatsapp">واتساب</option><option value="phone">اتصال</option><option value="email">بريد إلكتروني</option></select></Field>
              <Field label="الإمارة"><select value={form.emirate} onChange={(e) => patch('emirate', e.target.value)} className="input"><option value="">اختر الإمارة</option>{emirates.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field>
              <Field label="المدينة / المنطقة"><input value={form.city} onChange={(e) => patch('city', e.target.value)} className="input" placeholder="مثال: العين، الشامخة، دبي" /></Field>
            </div>

            {isQuote ? (
              <div className="grid gap-4 md:grid-cols-2">
                <Field label="نوع الخدمة"><select value={form.service_category} onChange={(e) => patch('service_category', e.target.value)} className="input"><option value="">اختر نوع الخدمة</option>{serviceCategories.map((item) => <option key={item} value={item}>{item}</option>)}</select></Field>
                <Field label="نوع المشروع"><input value={form.project_type} onChange={(e) => patch('project_type', e.target.value)} className="input" placeholder="مثال: فيلا، شقة، محل، مكتب" /></Field>
                <Field label="المساحة / الكمية"><input value={form.project_area} onChange={(e) => patch('project_area', e.target.value)} className="input" placeholder="مثال: 120 متر أو 8 أبواب" /></Field>
                <Field label="الميزانية التقريبية"><input value={form.budget_range} onChange={(e) => patch('budget_range', e.target.value)} className="input" placeholder="اختياري" /></Field>
                <Field label="موعد التنفيذ"><input value={form.timeline} onChange={(e) => patch('timeline', e.target.value)} className="input" placeholder="مثال: خلال أسبوعين" /></Field>
                <div className="md:col-span-2"><Field label="تفاصيل الطلب" required><textarea value={form.project_description} onChange={(e) => patch('project_description', e.target.value)} className="input min-h-[150px]" placeholder="اكتب تفاصيل العمل المطلوب، المقاسات، الموقع، أي ملاحظات مهمة..." /></Field></div>
              </div>
            ) : (
              <div className="space-y-4"><Field label="موضوع الاستفسار"><input value={form.inquiry_topic} onChange={(e) => patch('inquiry_topic', e.target.value)} className="input" placeholder="مثال: طريقة طلب خدمة / التسجيل / الأسعار" /></Field><Field label="الرسالة" required><textarea value={form.message} onChange={(e) => patch('message', e.target.value)} className="input min-h-[150px]" placeholder="اكتب استفسارك هنا..." /></Field></div>
            )}

            <div className="rounded-2xl border border-dashed border-[#D4AF37]/60 bg-[#FFF8E7] p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div><p className="text-sm font-black text-[#0F3F1A]">إرفاق صور</p><p className="mt-1 text-xs font-bold leading-6 text-gray-600">اختياري — أضف صورًا للمكان، المقاسات، الخامة، أو المشكلة. الحد الأقصى {MAX_ATTACHMENTS} صور.</p></div><label className="inline-flex cursor-pointer items-center justify-center gap-2 rounded-2xl bg-white px-5 py-3 text-sm font-black text-[#0F3F1A] shadow-sm ring-1 ring-[#E6DCC8]"><ImagePlus className="h-5 w-5 text-[#B0912F]" />اختيار صور<input type="file" accept="image/*" multiple onChange={handleFiles} className="hidden" /></label></div>
              ...
              {attachments.length > 0 ? <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">{attachments.map((item, index) => <div key={`${item.name}-${index}`} className="relative overflow-hidden rounded-2xl border border-[#E6DCC8] bg-white p-2"><img src={item.data_url} alt={item.name} className="h-24 w-full rounded-xl object-cover" /><button type="button" onClick={() => removeAttachment(index)} className="absolute left-3 top-3 flex h-7 w-7 items-center justify-center rounded-full bg-white/95 text-[#0F3F1A] shadow"><X className="h-4 w-4" /></button><p className="mt-2 truncate text-[11px] font-bold text-gray-600" title={item.name}>{item.name}</p></div>)}</div> : null}
            </div>

            <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-[#E6DCC8] bg-[#FDFBF7] p-4 text-sm font-bold leading-7 text-gray-700"><input type="checkbox" checked={form.accepted} onChange={(e) => patch('accepted', e.target.checked)} className="mt-1 h-5 w-5 accent-[#0F3F1A]" /><span>أوافق على استخدام بياناتي والصور المرفقة للتواصل بخصوص هذا الطلب فقط، وفق سياسة الخصوصية في بيت الريف.</span></label>
