...
const profileWhatsapp = buildProviderWhatsappUrl({
  ...
  locale: 'ar',
  providerName: provider.identity.name.ar,
  providerCode: provider.id,
  location: 'العين · أبوظبي · دبي',
  summary: provider.description.ar,
  profilePath: `/providers/${provider.slug}`,
});
    ...
    <>
      ...
      <div dir="rtl" className="min-h-screen bg-[#F7FAFC] text-[#082E63]">
        ...
        <main>
                  ...
                  <div className="relative rounded-[2.2rem] border border-white/70 bg-white/95 p-5 shadow-2xl backdrop-blur-xl md:p-8">
                    <div className="flex flex-col items-center gap-5 text-center">
                      ...
                      <div className="flex w-full flex-col justify-center gap-3 sm:flex-row">
                        <a href={profileWhatsapp} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-[#55B51B] px-8 font-black text-white shadow-lg"><MessageCircle className="h-5 w-5" /> واتساب</a>
          ...
          <section className="px-4 py-16"><div className="mx-auto max-w-6xl rounded-[2.5rem] bg-[linear-gradient(135deg,#55B51B,#0870C2)] p-8 text-center text-white shadow-2xl md:p-12"><h2 className="text-3xl font-black">أرسل الصور والمقاسات وموقع الخدمة</h2><p className="mx-auto mt-4 max-w-2xl leading-8 text-white/85">يساعد ذلك على تقييم الخدمة واختيار المعدات والمواد المناسبة وتأكيد الموعد.</p><a href={profileWhatsapp} target="_blank" rel="noopener noreferrer" className="mt-7 inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-white px-9 font-black text-[#073D7C]"><MessageCircle className="h-5 w-5" /> تواصل مع الرحاب</a></div></section>
        </main>
        <Footer />
      </div>
    </>
...
function ContentCard({ item }) { const message = buildCardWhatsappUrl({ phone: whatsappDigits, locale: 'ar', cardType: 'service', providerName: provider.identity.name.ar, providerCode: provider.id, cardCode: item.id, cardId: item.id, title: item.title.ar, description: item.summary.ar, pricingModel: 'السعر بعد التقييم', location: 'العين · أبوظبي · دبي', pagePath: `/providers/${provider.slug}#${item.id}` }); return <article id={item.id} className="overflow-hidden rounded-3xl border border-[#DDEAF7] bg-white shadow-sm"><div className="relative aspect-[4/3]"><Image src={item.image} alt={item.title.ar} fill className="object-cover" /></div><div className="p-5"><div className="flex items-center justify-between gap-2"><CheckCircle2 className="h-6 w-6 text-[#55B51B]" /><span className="text-xs font-black text-[#0B5EAE]" dir="ltr">{item.id}</span></div><h3 className="mt-3 text-xl font-black text-[#073D7C]">{item.title.ar}</h3><p className="mt-3 text-sm leading-7 text-slate-600">{item.summary.ar}</p><a href={message} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#55B51B] px-4 text-sm font-black text-white"><MessageCircle className="h-4 w-4" />طلب الخدمة عبر واتساب</a></div></article>; }
function OfferCard({ offer }) { const message = buildCardWhatsappUrl({ phone: whatsappDigits, locale: 'ar', cardType: 'offer', providerName: provider.identity.name.ar, providerCode: provider.id, cardCode: offer.id, cardId: offer.id, title: offer.title.ar, description: offer.summary.ar, price: 'بعد التقييم', pricingModel: 'عرض سعر بعد مراجعة الصور والكمية والموقع', location: 'العين · أبوظبي · دبي', pagePath: `/providers/${provider.slug}#${offer.id}` }); return <article id={offer.id} className="overflow-hidden rounded-3xl border border-white/15 bg-white/10 backdrop-blur"><div className="relative aspect-[4/3]"><Image src={offer.image} alt={offer.title.ar} fill className="object-cover" /><span className="absolute right-3 top-3 rounded-full bg-[#55B51B] px-3 py-1 text-xs font-black text-white">{offer.badge.ar}</span></div><div className="p-5"><p className="text-xs font-black text-[#89D13C]" dir="ltr">{offer.id}</p><h3 className="mt-2 text-xl font-black">{offer.title.ar}</h3><p className="mt-3 text-sm leading-7 text-white/70">{offer.summary.ar}</p><p className="mt-4 text-xs font-black text-[#89D13C]">السعر بعد التقييم</p><a href={message} target="_blank" rel="noopener noreferrer" className="mt-5 inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#55B51B] px-4 text-sm font-black text-white"><MessageCircle className="h-4 w-4" />استفسر عبر واتساب</a></div></article>; }
