...
const EN_PLATFORM_LINKS = [
  ...
  { key: 'marketplace', label: 'Products & Stores', href: 'https://bietalreef.ae/en/marketplace' },
  { key: 'provider_register', label: 'Join as a provider', href: 'https://bietalreef.ae/en/providers/register' },
...
export default async function weyaakAgentHandler(req, res) {
  ...
  try {
    ...
    return res.status(200).json({
      reply: cleanReply(agent.reply) || (locale === 'en' ? 'I understand. Tell me the next detail and I will continue.' : 'وصلت فكرتك. خبرني بالنقطة التالية عشان أكملها معك.'),
