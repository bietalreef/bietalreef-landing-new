...
const productActions = [
  ...
  { ar: 'اطلب تسعير مواد', en: 'Request material pricing', hrefAr: '/request-quote', hrefEn: '/en/request-quote', icon: '📋' },
  ...
  return (
    <section dir={dir} className="bg-[#FDFBF7] px-4 py-12 md:py-16">
      <div className="mx-auto max-w-7xl">
        <div className="mb-7 text-center"><p className="text-sm font-black text-[#B8922B]">{isEn ? 'Products gateway' : 'بوابة المنتجات والمتاجر'}</p><h2 className="mt-2 text-2xl font-black leading-tight text-[#0F3F1A] md:text-4xl">{isEn ? 'Smart footer for Products & Stores' : 'فوتر ذكي خاص بالمنتجات والمتاجر'}</h2><p className="mx-auto mt-3 max-w-3xl text-sm font-semibold leading-7 text-gray-600 md:text-base">{isEn ? 'A dedicated products footer that connects materials, stores, suppliers, pricing requests and related marketplace paths.' : 'فوتر مستقل للمنتجات والمتاجر يربط المواد، المتاجر، الموردين، طلبات التسعير، ومسارات الشراء والتوريد المرتبطة.'}</p></div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-3"><FooterCard defaultOpen title={isEn ? 'Product categories' : 'فئات المنتجات'} subtitle={isEn ? 'Materials and products without page crowding' : 'مواد ومنتجات بدون ازدحام بصري'} icon="📦"><LinkList locale={locale} items={productGroups} /></FooterCard><FooterCard defaultOpen title={isEn ? 'Sourcing journey' : 'مسار طلب التوريد'} subtitle={isEn ? 'From product need to supplier path' : 'من احتياج المادة إلى مسار المورد'} icon="🧭"><LinkList locale={locale} items={productJourney} /></FooterCard><FooterCard title={isEn ? 'Related platform sections' : 'أقسام مرتبطة داخل المنصة'} subtitle={isEn ? 'Services, providers and UAE directory' : 'الخدمات، المزودون، ودليل الإمارات'} icon="🏗️"><LinkList locale={locale} items={relatedLinks} /></FooterCard></div>
