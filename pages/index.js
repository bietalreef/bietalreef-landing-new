...
const gatewayCards = [
  { title: 'دليل الإمارات', desc: 'ابدأ من المكان: الإمارة، المدينة، المنطقة، ثم الخدمة المناسبة لمشروعك.', href: '/uae', icon: MapPinned, label: 'بحث حسب المكان', image: '/images/uae-atlas/construction-platform-clear-path.webp', imageAlt: 'منصة البناء الذكية ومسار دليل الإمارات داخل بيت الريف' },
        ...
        <main className="-mt-[1px]">
          ...
          <section className="mx-auto max-w-7xl px-4 py-8 md:py-12">
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
              {gatewayCards.map((card) => { const Icon = card.icon; return <Link key={card.href} href={card.href} className="group relative overflow-hidden rounded-[2rem] border border-[#E6DCC8] bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl"><div className="absolute -left-10 -top-10 h-28 w-28 rounded-full bg-[#D4AF37]/10 transition group-hover:scale-125" />{card.image ? <div className="relative h-48 overflow-hidden bg-[#F7F2E8]"><Image src={card.image} alt={card.imageAlt || card.title} fill className="scale-105 object-cover object-center transition duration-700 group-hover:scale-110" sizes="(max-width: 768px) 92vw, (max-width: 1280px) 45vw, 25vw" /><div className="absolute inset-0 bg-gradient-to-b from-white/5 via-transparent to-white/25" /></div> : null}<div className="relative p-7"><div className="mb-6 flex items-center justify-between"><div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-[#0F3F1A] text-white shadow-lg shadow-[#0F3F1A]/15"><Icon className="h-7 w-7" aria-hidden="true" /></div><span className="rounded-full bg-[#F7F2E8] px-3 py-1 text-xs font-black text-[#6F5400]">{card.label}</span></div><h3 className="text-2xl font-black text-[#0F3F1A]">{card.title}</h3><p className="mt-3 min-h-[88px] text-sm leading-8 text-gray-600">{card.desc}</p><span className="mt-6 inline-flex items-center gap-2 text-sm font-black text-[#6F5400] transition group-hover:-translate-x-1">افتح القسم<ArrowLeft className="h-4 w-4" aria-hidden="true" /></span></div></Link>; })}
            </div>
          </section>
