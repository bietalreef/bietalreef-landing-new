...
function safeLinks(raw, trace, official, message, intent) {
  ...
  for (const provider of providers) {
    if (provider.url && !safe.some((link) => link.href === provider.url)) {
      safe.push({
        label: provider.verified ? `ملف مزود موثّق — ${provider.name}` : `ملف مزود منشور — ${provider.name}`,
        href: provider.url,
      });
    }
  }
...
function isDirectCustomerSearch(message) {
  ...
  const asksAvailability = /(هل\s*(?:يوجد|في|عندكم)|وين|أين|ابحث|أبحث|احتاج|أحتاج|اريد|أريد|ابغى|دلني|عطني)/i.test(value);
  const asksForTrade = /(نجار|فني|مقاول|سباك|كهربائي|شركة\s+(?:تنظيف|صيانة|مقاولات)|ورشة|مصنع|مزود\s+خدمة|رخام|جرانيت|تكييف|ألمنيوم|المنيوم|زجاج|دهان)/i.test(value);
  const providerOnboarding = /(انضم|انضمام|اشتراك|سجل\s+شركتي|تسجيل\s+شركتي|إضافة\s+نشاطي|اضافة\s+نشاطي)/i.test(value);
  return asksAvailability && asksForTrade && !providerOnboarding;
}
