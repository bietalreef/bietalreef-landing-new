...
function isProviderSelfIdentification(value) {
  return PROVIDER_SELF_WORDS.test(String(value || '').trim());
}

function isDirectProviderSearch(value) {
  const raw = String(value || '').trim();
  if (isProviderSelfIdentification(raw)) return false;
...
export function applyAnswerToWeyaakState(state = {}, message = '') {
  ...
  if (isProviderSelfIdentification(value)) {
    return { audience: 'provider', intent: 'provider_subscription', payload: { ...next.payload } };
  }
