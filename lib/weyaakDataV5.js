...
const SUPPORT_NUMBER = '971564456337';
...
const PAYLOAD_FIELDS = [
  ...
  'business_name', 'business_location', 'specialty', 'service_areas', 'license_status', 'portfolio_status',
];
...
function directServiceTerms(value) {
...
async function loadProviders() {
  ...
  const [profiles, services, locations] = await Promise.all([
    supabaseGet('provider_public_profiles?select=id,public_provider_code,slug,name_ar,name_en,provider_type,short_description_ar,short_description_en,description_ar,description_en,canonical_url,verification_status,search_keywords_ar,search_keywords_en&publication_status=eq.published&limit=100'),
...
function publicProviderResult(provider, locale = 'ar') {
  const isEnglish = locale === 'en';
  return {
    provider_code: provider.public_provider_code || '',
    slug: provider.slug,
    name: isEnglish
      ? (provider.name_en || provider.name_ar)
      : (provider.name_ar || provider.name_en),
    name_ar: provider.name_ar || provider.name_en || '',
    name_en: provider.name_en || provider.name_ar || '',
    url: `${SITE_URL}${isEnglish ? '/en' : ''}/providers/${provider.slug}`,
    url_ar: `${SITE_URL}/providers/${provider.slug}`,
    url_en: `${SITE_URL}/en/providers/${provider.slug}`,
    verified: provider.verification_status === 'verified',
    summary: isEnglish
      ? (provider.short_description_en || provider.description_en || provider.short_description_ar || provider.description_ar || '')
      : (provider.short_description_ar || provider.description_ar || provider.short_description_en || provider.description_en || ''),
    ...
    coverage: (provider.locations || []).map((location) => ({
      ...
      emirate: isEnglish
        ? (location.emirate?.name_en || location.emirate?.name_ar || '')
        : (location.emirate?.name_ar || location.emirate?.name_en || ''),
      city: isEnglish
        ? (location.city?.name_en || location.city?.name_ar || '')
        : (location.city?.name_ar || location.city?.name_en || ''),
      area: isEnglish
        ? (location.area?.name_en || location.area?.name_ar || '')
        : (location.area?.name_ar || location.area?.name_en || ''),
    })),
  };
}
...
export async function searchProviders({ service, city = '', emirate = '' }) {
  ...
  const requiredDirectTerms = directServiceTerms(requestedService);
  ...
  const matches = providers
    .map((provider) => {
      ...
      // Expanded family terms improve ranking, but at least one term written by
      // the user must match. This prevents "نجار" from matching a marble
      // provider merely because both mention kitchen work.
      const specialtyMatched = requiredDirectTerms.some((term) => serviceText.includes(term));
      ...
      return {
        ...
        provider: {
          ...publicProviderResult(provider, 'ar'),
...
export async function searchProvidersByLocation({ city = '', emirate = '', locale = 'ar' }) {
  const requestedCity = text(city || '', 120);
  const requestedEmirate = text(emirate || '', 120);
  const providers = await loadProviders();
  const matches = providers
    .filter((provider) => locationMatches(provider, requestedCity, requestedEmirate))
    .map((provider) => publicProviderResult(provider, locale))
    .slice(0, 5);

  return {
    status: matches.length ? 'matched' : 'unmatched',
    query: { city: requestedCity, emirate: requestedEmirate },
    providers: matches,
  };
}
  ...
  if (type === 'provider_interest') {
    return {
      ...
      fields: [
        ...
        { name: 'business_location', label: 'مقر الشركة أو الورشة', type: 'text', required: true },
...
export async function submitAction(action, meta = {}) {
  ...
  const providerMessage = [
    ...
    payload.business_location ? `مقر النشاط: ${payload.business_location}` : '',
...
export function whatsappUrl(type, payload, number) {
  const lines = type === 'provider_interest'
    ? [
        ...
        `مقر النشاط: ${payload.business_location}`,
